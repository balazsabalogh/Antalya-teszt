...
const LOCAL_REAL={konyaalti:'./assets/photos/konyaalti.jpg',hadrian:'./assets/photos/hadrian.jpg',duden:'./assets/photos/duden.png',perge:'./assets/photos/perge.jpg',kursunlu:'./assets/photos/kursunlu.jpg',phaselis:'./assets/photos/phaselis.jpg',hotel:'./assets/generated/hotel.jpg',firinci:'./assets/generated/coffee.jpg',migros:'./assets/generated/shop.jpg',migros5m:'./assets/generated/shop.jpg',sisci:'./assets/generated/food.jpg',wc1:'./assets/generated/wc.jpg'};
const GENERIC_PHOTO={historic:'./assets/generated/historic.jpg',museum:'./assets/generated/museum.jpg',beach:'./assets/generated/beach.jpg',park:'./assets/generated/park.jpg',waterfall:'./assets/generated/waterfall.jpg',ancient:'./assets/generated/ancient.jpg',food:'./assets/generated/food.jpg',coffee:'./assets/generated/coffee.jpg',bar:'./assets/generated/bar.jpg',shop:'./assets/generated/shop.jpg',wc:'./assets/generated/wc.jpg',hotel:'./assets/generated/hotel.jpg',attraction:'./assets/generated/attraction.jpg',marina:'./assets/generated/marina.jpg'};
...
function offlinePhoto(p){return LOCAL_REAL[p.id]||GENERIC_PHOTO[p.type]||placeholderFor(p)}
...
async function hydrateOnlineImages(root=document){
 ...
 await Promise.all(imgs.map(async img=>{
  ...
  img.onerror=()=>{img.onerror=null;img.src=fallback||'./assets/generated/generic.jpg';img.dataset.online='0';localStorage.removeItem('tc_wiki_photo_'+id);const badge=img.closest('.photo-wrap')?.querySelector('.photo-badge');if(badge)badge.hidden=true};
...
function readNumber(key,fallback,min=-Infinity,max=Infinity){const raw=localStorage.getItem(key);if(raw===null||raw==='')return fallback;const n=Number(raw);return Number.isFinite(n)?Math.max(min,Math.min(max,n)):fallback}
function readEnum(key,allowed,fallback){const v=localStorage.getItem(key);return allowed.includes(v)?v:fallback}
const DEFAULT_FILTERS=['historic','food','coffee','bar','park','beach','shop','wc'];
const DAYS=[
...
{id:2,date:'08.26.',title:'Konyaaltı · strand',color:dayColors[1],hero:'konyaalti',summary:'Laza strandnap, Beach Park és naplemente.',route:['firinci','konyaalti','beachpark','ataturk'],optional:['aquarium'],transport:'Gyalog + tömegközlekedés',pace:2},
{id:3,date:'08.27.',title:'Kaleiçi · történelmi Antalya',color:dayColors[2],hero:'kaleici',summary:'Óváros, kapu, kikötő és tengerparti park.',route:['hadrian','kaleici','marina','hidirlik','karaali'],optional:['sisci','wc1'],transport:'Gyalog',pace:3},
...
const ROUTE_SCHEMA='2026-08-25-r1';
const savedSchema=localStorage.getItem('tc_route_schema')||'';
const SAVED_ROUTES=readJSON('tc_routes',null);
if(Array.isArray(SAVED_ROUTES)){
 DAYS.forEach((d,i)=>{
  const r=SAVED_ROUTES[i];
  if(Array.isArray(r)){
   const clean=[...new Set(r.filter(id=>POIS[id]&&id!=='museum'))];
   if(clean.length)d.route=clean;
  }
 });
}
// Current official status: Antalya Archaeology Museum is closed, so never keep it in a live route.
DAYS.forEach(d=>{d.route=d.route.filter(id=>id!=='museum');d.optional=d.optional.filter(id=>id!=='museum')});
localStorage.setItem('tc_route_schema',ROUTE_SCHEMA);
const SAVED_DAY=Math.round(readNumber('tc_day',0,0,DAYS.length-1));
const safeIds=a=>Array.isArray(a)?a.filter(id=>POIS[id]):[];
const savedFilters=readJSON('tc_filters',DEFAULT_FILTERS);
let state={day:SAVED_DAY,selected:null,sheet:'closed',view:'poi',sonar:false,sonarPins:[],favorites:new Set(safeIds(readJSON('tc_favs',[]))),done:new Set(safeIds(readJSON('tc_done',[]))),skipped:new Set(safeIds(readJSON('tc_skipped',[]))),theme:readEnum('tc_theme',['dark','light','auto'],'dark'),pace:readNumber('tc_pace',3,1,5),transport:readEnum('tc_transport',['Gyalog','Tömegközlekedés','Autó','Vegyes'],'Vegyes'),arrivalDetect:localStorage.getItem('tc_arrival_detect')!=='0',notifications:[
...
],nav:false,navVisible:false,userPos:null,adding:false,notifStripDismissed:false,filters:new Set((Array.isArray(savedFilters)?savedFilters:DEFAULT_FILTERS).filter(t=>DEFAULT_FILTERS.includes(t))),sonarRadius:readNumber('tc_sonar_radius',0.5,0.25,2),activeMenu:null,planSnapshots:readJSON('tc_plan_snapshots',[])};
if(!Array.isArray(state.planSnapshots))state.planSnapshots=[];
...
function save(){localStorage.setItem('tc_favs',JSON.stringify([...state.favorites]));localStorage.setItem('tc_done',JSON.stringify([...state.done]));localStorage.setItem('tc_skipped',JSON.stringify([...state.skipped]));localStorage.setItem('tc_day',String(state.day));localStorage.setItem('tc_routes',JSON.stringify(DAYS.map(d=>d.route)));localStorage.setItem('tc_route_schema',ROUTE_SCHEMA);}
...
function syncActiveMenus(){
 ...
 const mainMap={itinerary:'route',days:'days',favorites:'favorites',settings:'settings',notifications:'notifications',navigation:'navigation'};
...
let navRouteDetail=null;
function routeEndpoint(){const t=(state.transport||currentDay().transport||'').toLowerCase();return t.includes('gyalog')?'routed-foot':t.includes('autó')?'routed-car':'routed-car'}
async function fetchNavigationRoute(coords){
 if(coords.length<2)return {geometry:coords,distance:0,duration:0,steps:[]};
 if(!navigator.onLine)return {geometry:coords,distance:km({lat:coords[0][0],lon:coords[0][1]},{lat:coords.at(-1)[0],lon:coords.at(-1)[1]})*1000,duration:0,steps:[]};
 try{
  const q=coords.map(c=>`${c[1]},${c[0]}`).join(';'),srv=routeEndpoint();
  const r=await fetch(`https://routing.openstreetmap.de/${srv}/route/v1/driving/${q}?overview=full&geometries=geojson&steps=true`);
  if(!r.ok)throw new Error('route');const j=await r.json(),route=j.routes?.[0];if(!route)throw new Error('route');
  const steps=(route.legs||[]).flatMap(l=>l.steps||[]);
  return {geometry:route.geometry?.coordinates?.map(c=>[c[1],c[0]])||coords,distance:route.distance||0,duration:route.duration||0,steps};
 }catch(_){return {geometry:coords,distance:km({lat:coords[0][0],lon:coords[0][1]},{lat:coords.at(-1)[0],lon:coords.at(-1)[1]})*1000,duration:0,steps:[]}}
}
function navDirection(step){const m=step?.maneuver||{},type=m.type||'',mod=m.modifier||'',name=(step.name||'').trim();let a='Haladj tovább';
 if(type==='depart')a='Indulj el';else if(type==='arrive')a='Megérkeztél';else if(type==='roundabout'||type==='rotary')a='Hajts be a körforgalomba';else if(type==='merge')a='Sorolj be';else if(type==='fork')a=mod.includes('left')?'Tarts balra':'Tarts jobbra';else if(type==='turn'||type==='end of road'||type==='new name'){
  if(mod==='left')a='Fordulj balra';else if(mod==='right')a='Fordulj jobbra';else if(mod==='sharp left')a='Fordulj élesen balra';else if(mod==='sharp right')a='Fordulj élesen jobbra';else if(mod==='slight left')a='Tarts enyhén balra';else if(mod==='slight right')a='Tarts enyhén jobbra';else if(mod==='uturn')a='Fordulj vissza';
 }
 return name&&type!=='arrive'?`${a} · ${name}`:a;
}
function durationText(sec){if(!sec)return '—';const min=Math.max(1,Math.round(sec/60));return min<60?`${min} perc`:`${Math.floor(min/60)} ó ${min%60?min%60+' perc':''}`.trim()}
function navStepsHTML(){const p=POIS[state.selected],d=navRouteDetail?.id===state.selected?navRouteDetail:null;if(!p)return '<div class="setting-card">Nincs aktív cél.</div>';if(!d)return '<div class="setting-card nav-loading">Útvonal részleteinek betöltése…</div>';
 const summary=`<div class="nav-steps-summary"><div><small>Aktív cél</small><b>${esc(p.name)}</b></div><div><small>Távolság</small><b>${distText((d.distance||0)/1000)}</b></div><div><small>Becsült idő</small><b>${durationText(d.duration)}</b></div></div>`;
 const steps=d.steps?.length?`<div class="nav-step-list">${d.steps.filter(s=>s.distance>2||s.maneuver?.type==='arrive').map((s,i)=>`<div class="nav-step"><span>${i+1}</span><div><b>${esc(navDirection(s))}</b><small>${distText((s.distance||0)/1000)}${s.duration?` · ${durationText(s.duration)}`:''}</small></div></div>`).join('')}</div>`:`<div class="setting-card">A részletes lépések most nem érhetők el. A térképes navigáció továbbra is működik.</div>`;
 return `${summary}<button class="nav-map-return" data-nav-map>← Térképnézet</button>${steps}`;
}
function closeNavigationSteps(){hardCloseOverlay();document.body.classList.remove('overlay-fullscreen');state.activeMenu='navigation';if(state.nav&&state.selected){state.navVisible=true;renderNavBanner(state.selected);drawNavigationRoute(state.selected)}syncActiveMenus();$('#overlayClose').onclick=closeOverlay;$('#overlayBack').onclick=closeOverlay}
function showNavigationSteps(){if(!state.nav||!state.selected){toast('Előbb indíts Companion navigációt');return}openOverlay('Navigáció · Lépések',POIS[state.selected].name,navStepsHTML());overlay.classList.add('nav-steps-mode');document.body.classList.add('overlay-fullscreen');$('#overlayClose').onclick=closeNavigationSteps;$('#overlayBack').onclick=closeNavigationSteps;const b=$('[data-nav-map]');if(b)b.onclick=closeNavigationSteps;if(!navRouteDetail||navRouteDetail.id!==state.selected)drawNavigationRoute(state.selected)}

function renderDayMap(fit=false){
  if(!map||!markerLayer||!sonarLayer||!filterLayer)return;
...
function selectPoi(id){if(state.adding&&!currentDay().route.includes(id)){addAsNext(id);exitAddMode();return}state.selected=id;if(state.sonarPins.includes(id))pauseSonarExpiry();state.view='poi';setSheet('mini');renderDayMap(false);const p=POIS[id];setTimeout(()=>{if(map)map.panTo([p.lat,p.lon],{animate:true})},80)}
function fitRoute(){if(!map){toast('A térkép első online indítás után érhető el');return}const pts=currentDay().route.map(id=>[POIS[id].lat,POIS[id].lon]);if(pts.length)map.fitBounds(L.latLngBounds(pts).pad(.25),{maxZoom:14})}
let gpsWatchId=null,lastNavRouteAt=0,lastNavRouteOrigin=null,navRouteToken=0;
function updateUserPosition(pos,recenter=false){
 state.userPos={lat:pos.coords.latitude,lon:pos.coords.longitude};updateTestBadge();
 const o=effectiveOrigin();
 if(map){if(userMarker)map.removeLayer(userMarker);userMarker=L.circleMarker([o.lat,o.lon],{radius:8,color:'#fff',weight:3,fillColor:'#357ee9',fillOpacity:1}).addTo(map);if(recenter)map.setView([o.lat,o.lon],15)}
 checkArrival();
 if(state.nav&&state.selected){renderNavBanner(state.selected);const moved=!lastNavRouteOrigin||km(lastNavRouteOrigin,o)>.025;if(moved&&Date.now()-lastNavRouteAt>10000){lastNavRouteAt=Date.now();lastNavRouteOrigin={lat:o.lat,lon:o.lon};drawNavigationRoute(state.selected)}}
}
function locate(){if(!navigator.geolocation){toast('A GPS nem elérhető');return}navigator.geolocation.getCurrentPosition(pos=>updateUserPosition(pos,true),()=>toast('A helyzethez engedélyezd a GPS-t'),{enableHighAccuracy:true,timeout:8000,maximumAge:3000})}
function startGpsWatch(){if(!navigator.geolocation||gpsWatchId!==null)return;gpsWatchId=navigator.geolocation.watchPosition(pos=>updateUserPosition(pos,false),()=>{}, {enableHighAccuracy:true,timeout:12000,maximumAge:4000})}
function stopGpsWatch(){if(gpsWatchId!==null&&navigator.geolocation){navigator.geolocation.clearWatch(gpsWatchId);gpsWatchId=null}lastNavRouteOrigin=null;lastNavRouteAt=0;navRouteToken++}
function checkArrival(){if(!state.arrivalDetect||!state.userPos||!gpsInTrip())return;const id=currentDay().route.find(x=>!state.done.has(x)&&!state.skipped.has(x));if(!id)return;const d=km(state.userPos,POIS[id]);if(d<.1&&!state.notifications.some(n=>n.id==='arrive-'+id)){state.notifStripDismissed=false;state.notifications.unshift({id:'arrive-'+id,priority:'important',title:'Megérkeztetek',text:POIS[id].name+' · Megnéztük?',action:'arrival',poi:id});renderNotifications()}}

const sheetHeights={closed:0,mini:132,one:()=>Math.round(innerHeight*.34),two:()=>Math.round(innerHeight*.50),full:()=>innerHeight};
function sonarPanelHeight(){const rows=Math.min(Math.max(state.sonarPins.length,1),3);return Math.min(Math.round(innerHeight*.48),250+rows*82)}
function heightFor(s){
 if(s==='two'&&state.view==='sonar')return sonarPanelHeight();
 if(s==='two'&&['route','poi','add','savedadd'].includes(state.view))return Math.round(innerHeight*.50);
 const v=sheetHeights[s];return typeof v==='function'?v():v
}
function setSheet(s,view){
 if(view)state.view=view;
 if(state.view==='route'&&s==='one')s='two';
 if(state.view==='sonar'&&(s==='one'||s==='full'))s='two';
 if(state.view==='poi'&&s==='one')s='two';
 if(s!=='closed'&&typeof closeCompactIfOpen==='function')closeCompactIfOpen();
 state.sheet=s;
 document.body.classList.toggle('sheet-fullscreen',s==='full');
 if(s!=='closed'&&state.view==='route')state.activeMenu='route';
 else if(s!=='closed'&&state.view==='sonar')state.activeMenu='sonar';
 else if(s==='closed'&&['route','sonar'].includes(state.activeMenu)&&!state.nav)state.activeMenu=null;
 const h=heightFor(s);document.documentElement.style.setProperty('--sheet-h',h+'px');
 ...
 mainNav.classList.toggle('joined-panel',s!=='closed'&&s!=='full');
 const ctr=$('#mapControls');const large=['two','full'].includes(s);ctr.classList.toggle('hidden-by-sheet',large);ctr.style.bottom='';
 renderSheet();
 ...
 if(s==='full'){
   ...
   x.onclick=()=>{if(state.view==='guide'){state.view='poi';setSheet('mini')}else if(state.view==='route'){setSheet('two','route')}else setSheet('closed')};sheet.appendChild(x)
 }
...
const PANEL_NAV_ITEMS=[['route','Útiterv','route'],['days','Napok','days'],['heart','Kedvencek','favorites'],['settings','Beáll.','settings'],['bell','Értes.','notifications'],['nav','Navigáció','navigation']];
...
function imgTag(p,cls=''){const src=offlinePhoto(p);return `<span class="photo-wrap ${cls}"><img data-poi-img="${p.id}" data-fallback="${src}" src="${src}" alt="${esc(p.name)}" onerror="this.onerror=null;this.src='./assets/generated/generic.jpg'"><span class="photo-badge" hidden>Wikimedia</span></span>`}
...
function poiActions(id){const inRoute=currentDay().route.includes(id);return `<div class="actions action-carousel poi-primary-actions">${iconButton('ⓘ','Guide','guide')}${inRoute?iconButton('−','Kivétel','danger subtle'):iconButton('+','Útitervhez','add')}${iconButton('➤','Navigáció','primary')}${iconButton(state.favorites.has(id)?'♥':'♡','Kedvenc')}${iconButton('↗','Megosztás')}</div>`}
...
function bindPlannerGestures(){$$('.planner-swipe').forEach(wrap=>{const card=wrap.querySelector('.planner-card');let sx=0,sy=0,dx=0,drag=false,moved=false,axis=null;
 const reset=()=>{drag=false;dx=0;axis=null;card.style.transform='';wrap.classList.remove('swipe-left','swipe-right','is-swiping')};
 card.addEventListener('pointerdown',e=>{if(e.target.closest('.drag'))return;sx=e.clientX;sy=e.clientY;dx=0;drag=true;moved=false;axis=null;card.setPointerCapture?.(e.pointerId)});
 card.addEventListener('pointermove',e=>{if(!drag)return;const mx=e.clientX-sx,my=e.clientY-sy;if(!axis&&Math.hypot(mx,my)>7)axis=Math.abs(mx)>Math.abs(my)*1.15?'x':'y';if(axis!=='x')return;dx=mx;moved=true;const x=Math.max(-118,Math.min(118,dx));card.style.transform=`translateX(${x}px)`;wrap.classList.add('is-swiping');wrap.classList.toggle('swipe-right',x>0);wrap.classList.toggle('swipe-left',x<0)});
 card.addEventListener('pointerup',()=>{if(!drag)return;const id=card.dataset.plan;const acted=axis==='x'&&Math.abs(dx)>78;if(acted&&dx>0){snapshotPlan('Megnéztem előtt');state.done.add(id);state.skipped.delete(id);toast('✓ Megnézve')}else if(acted&&dx<0){snapshotPlan('Kihagyás előtt');state.skipped.add(id);state.done.delete(id);toast('Kihagyva')}reset();if(acted){save();renderSheet();renderDayMap()}});
 card.addEventListener('pointercancel',reset);card.addEventListener('lostpointercapture',()=>{if(drag)reset()});
 card.onclick=e=>{if(e.target.closest('.drag')||moved){moved=false;return}state.selected=card.dataset.plan;state.activeMenu='route';setSheet('mini','poi')}})}
function bindPlannerReorder(){
 $$('.planner-card .drag').forEach(handle=>{
  let wrap=null,parent=null,scrollBox=null,startOrder=null,pointerId=null,moved=false,lastY=0;
  const finish=()=>{
   if(!wrap)return;
   wrap.classList.remove('reordering');document.body.classList.remove('planner-reorder-active');
   const order=[...parent.querySelectorAll('.planner-swipe')].map(x=>x.dataset.planWrap);
   if(moved&&order.join('|')!==startOrder.join('|')){snapshotPlan('Átrendezés előtt');currentDay().route=[...order];save();renderDayMap(false);toast('✓ Sorrend frissítve');renderRouteSheet()}
   wrap=parent=scrollBox=null;pointerId=null;moved=false;
  };
  const move=e=>{
   if(!wrap||e.pointerId!==pointerId)return;
   lastY=e.clientY;e.preventDefault();
   const siblings=[...parent.querySelectorAll('.planner-swipe')].filter(x=>x!==wrap);
   let before=null;
   for(const s of siblings){const r=s.getBoundingClientRect();if(e.clientY<r.top+r.height/2){before=s;break}}
   if(before){if(wrap.nextElementSibling!==before)parent.insertBefore(wrap,before)}else if(parent.lastElementChild!==wrap)parent.appendChild(wrap);
   moved=true;
   if(scrollBox){const r=scrollBox.getBoundingClientRect(),edge=54;if(e.clientY<r.top+edge)scrollBox.scrollBy({top:-16,behavior:'auto'});else if(e.clientY>r.bottom-edge)scrollBox.scrollBy({top:16,behavior:'auto'})}
  };
  const up=e=>{if(!wrap||e.pointerId!==pointerId)return;window.removeEventListener('pointermove',move,{capture:true});window.removeEventListener('pointerup',up,{capture:true});window.removeEventListener('pointercancel',up,{capture:true});finish()};
  handle.onpointerdown=e=>{
   e.preventDefault();e.stopPropagation();wrap=handle.closest('.planner-swipe');parent=wrap?.parentElement;if(!wrap||!parent)return;
   scrollBox=parent.closest('.full-planner')||parent;startOrder=[...currentDay().route];pointerId=e.pointerId;moved=false;lastY=e.clientY;
   wrap.classList.add('reordering');document.body.classList.add('planner-reorder-active');handle.setPointerCapture?.(e.pointerId);navigator.vibrate?.(10);
   window.addEventListener('pointermove',move,{capture:true,passive:false});window.addEventListener('pointerup',up,{capture:true});window.addEventListener('pointercancel',up,{capture:true});
  };
 })
}
function bindRouteDaySwipe(){const el=$('.route-editor');if(!el)return;let sx=0,sy=0,armed=false;el.onpointerdown=e=>{if(e.target.closest('button,.planner-swipe'))return;sx=e.clientX;sy=e.clientY;armed=true;el.setPointerCapture?.(e.pointerId)};el.onpointerup=e=>{if(!armed)return;armed=false;const dx=e.clientX-sx,dy=e.clientY-sy;sx=0;if(Math.abs(dx)>55&&Math.abs(dx)>Math.abs(dy)*1.25){state.day=(state.day+(dx<0?1:-1)+DAYS.length)%DAYS.length;save();state.selected=null;renderDayMap(true);renderRouteSheet();toast(`${state.day+1}. nap`)}}}
...
function restoreLatestSnapshot(){const snap=state.planSnapshots[0];if(!snap){toast('Nincs korábbi mentés');return}DAYS.forEach((d,i)=>{const r=Array.isArray(snap.routes?.[i])?snap.routes[i].filter(id=>POIS[id]&&id!=='museum'):d.route;if(r.length)d.route=[...new Set(r)]});state.day=Math.max(0,Math.min(DAYS.length-1,Number.isFinite(+snap.day)?+snap.day:state.day));state.done=new Set(safeIds(snap.done||[]));state.skipped=new Set(safeIds(snap.skipped||[]));save();renderDayMap(true);renderSheet();toast('↶ Előző terv visszaállítva')}
function removeFromRoute(id){const r=currentDay().route;if(!r.includes(id))return;snapshotPlan('Kivétel előtt');currentDay().route=r.filter(x=>x!==id);save();toast('Kivéve az útitervből · mentés készült');renderDayMap(false);renderSheet()}
function addAsNext(id){const r=currentDay().route;if(r.includes(id)){toast('Már benne van a napi tervben');return}snapshotPlan('Hozzáadás előtt');const nextIndex=r.findIndex(x=>!state.done.has(x)&&!state.skipped.has(x));const insertAt=nextIndex<0?r.length:nextIndex+1;r.splice(insertAt,0,id);save();toast('✓ Hozzáadva következő állomásként');renderDayMap()}
...
function renderSonarPins(){if(!map||!sonarLayer)return;sonarLayer.clearLayers();state.sonarPins.forEach(id=>{const p=POIS[id];const m=L.marker([p.lat,p.lon],{icon:L.divIcon({className:'',html:sonarHTML(p),iconSize:[34,42],iconAnchor:[16,39]})}).addTo(sonarLayer);m.on('click',e=>{L.DomEvent.stopPropagation(e);selectPoi(id)})})}
function openSonarMode(refresh=true){state.activeMenu='sonar';state.sonar=true;pauseSonarExpiry();if(refresh)refreshSonarPins();else renderSonarPins();renderDayMap();state.view='sonar';setSheet('two');scheduleSonarExpiry();syncActiveMenus()}
...
function renderSonarSheet(){const labels=[[.25,'250 m'],[.5,'500 m'],[1,'1 km'],[2,'2 km']];document.documentElement.style.setProperty('--sonar-sheet-h',sonarPanelHeight()+'px');sheetBody.innerHTML=`<div class="sonar-layout"><div class="sonar-heading-row"><div><h2>Sonar · Körülötted</h2><small>${state.sonarPins.length} találat</small></div><div class="sonar-head-actions"><button class="sonar-saved" data-sonar-saved aria-label="Mentett helyek">♥</button><button class="sonar-close" data-sonar-close aria-label="Sonar bezárása">×</button></div></div><div class="choice-row sonar-radius sonar-radius-row">${labels.map(([v,l])=>`<button class="choice ${Math.abs(state.sonarRadius-v)<.01?'active':''}" data-radius="${v}">${l}</button>`).join('')}</div><div class="filters sonar-filters">${filterHTML()}</div><div class="sonar-results">${sonarListHTML()}</div></div>`;$('[data-sonar-saved]').onclick=()=>showFavorites('sonar');$('[data-sonar-close]').onclick=()=>{setSheet('closed');state.activeMenu=state.nav?'navigation':null;syncActiveMenus()};$$('[data-radius]').forEach(b=>b.onclick=()=>{state.sonarRadius=+b.dataset.radius;localStorage.setItem('tc_sonar_radius',state.sonarRadius);refreshSonarPins();scheduleSonarExpiry();renderSonarSheet();renderDayMap(false)});bindFilters();bindSonarSwipes();hydrateOnlineImages(sheetBody)}
...
function enterAddMode(){if(!map){toast('Térképről hozzáadáshoz egyszer online kell megnyitni a térképet');return}state.adding=true;state.view='add';setSheet('two');state.sonar=true;state.sonarPins=Object.keys(POIS).filter(id=>!currentDay().route.includes(id));renderDayMap();toast('Koppints a térképen egy helyre')}
...
function transitUrl(id,provider='apple'){
 const p=POIS[id];if(!p)return '';
 const dest=`${p.lat},${p.lon}`;
 if(provider==='google')return `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(dest)}&travelmode=transit`;
 return `https://maps.apple.com/?daddr=${encodeURIComponent(dest)}&dirflg=r`;
}
function openExternalTransit(id,provider='apple'){
 const p=POIS[id];if(!p)return;
 state.selected=id;sessionStorage.setItem('tc_resume_poi',id);
 state.view='poi';setSheet('mini','poi');renderDayMap(false);
 const url=transitUrl(id,provider);toast(provider==='google'?'Google Maps · tömegközlekedés':'Apple Maps · tömegközlekedés');
 setTimeout(()=>{window.location.href=url},120);
}
function showNavigationChoice(id){
 const p=POIS[id];if(!p)return;
 clearTransientPanels();state.selected=id;state.activeMenu='navigation';syncActiveMenus();
 showHold([
  {label:'Companion · térkép',icon:'➜',act:()=>startNavigation(id)},
  {label:'Apple · Tömegköz.',icon:'',act:()=>openExternalTransit(id,'apple')},
  {label:'Google · Tömegköz.',icon:'G',act:()=>openExternalTransit(id,'google')},
  {label:'Saját pozíció',icon:'◎',act:locate},
  {label:'Útvonal középre',icon:'⌖',act:()=>{if(!map){toast('A térkép első online indítás után érhető el');return}state.selected=id;renderDayMap(false);const o=effectiveOrigin();map.fitBounds(L.latLngBounds([[o.lat,o.lon],[p.lat,p.lon]]).pad(.3),{maxZoom:15})}},
  {label:'Mégse',icon:'×',act:returnHome}
 ],'wide');
}
function suspendNavBanner(){if(state.nav)hideNavigationUI()}
function startNavigation(id){if(!map){toast('Companion térképes navigációhoz egyszer online kell megnyitni a térképet');return}state.nav=true;state.navVisible=true;state.activeMenu='navigation';state.selected=id;state.view='navigation';setSheet('closed');updateTestBadge();const p=POIS[id],o=effectiveOrigin();map.fitBounds(L.latLngBounds([[o.lat,o.lon],[p.lat,p.lon]]).pad(.30),{maxZoom:16});renderNavBanner(id);drawNavigationRoute(id);startGpsWatch();syncActiveMenus();if(!state.userPos)locate()}
async function drawNavigationRoute(id){if(!map)return;const token=++navRouteToken,o=effectiveOrigin(),p=POIS[id];const detail=await fetchNavigationRoute([[o.lat,o.lon],[p.lat,p.lon]]);if(token!==navRouteToken||!state.nav||state.selected!==id)return;navRouteDetail={...detail,id};if(routeLayer)map.removeLayer(routeLayer);routeLayer=L.polyline(detail.geometry,{color:'#2f7fe2',weight:5,opacity:.92,lineCap:'round'}).addTo(map);if(overlay.classList.contains('nav-steps-mode')){overlayBody.innerHTML=navStepsHTML();const b=$('[data-nav-map]');if(b)b.onclick=closeNavigationSteps}}
function renderNavBanner(id){let el=$('#navBanner');if(!el){el=document.createElement('div');el.id='navBanner';el.className='nav-banner';document.body.appendChild(el)}const p=POIS[id],origin=effectiveOrigin(),k=km(origin,p),test=state.userPos&&!gpsInTrip();const mode=(state.transport||currentDay().transport||'Vegyes');const transit=mode.toLowerCase().includes('tömeg')?'<span class="nav-legs">Tömegközlekedési útvonalhoz válaszd az Apple vagy Google Maps opciót.</span>':'';el.classList.toggle('top-slot',unread()===0||state.notifStripDismissed);el.innerHTML=`<div class="nav-swipe-action nav-dismiss">✕ Navigáció vége</div><div class="nav-swipe-action nav-next">➜ Következő</div><div class="nav-banner-card"><div class="nav-arrow"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20V4M5 11l7-7 7 7"/></svg></div><div><b>${distText(k)} · ${esc(p.name)}</b><small>${test?'Szimulált pozíció':`Haladj a cél felé · ${walkText(k)}`} · ${esc(mode)}</small>${transit}</div></div>`;state.navVisible=true;el.classList.remove('hidden');bindNavBannerSwipe(el)}
function bindNavBannerSwipe(el){const card=el.querySelector('.nav-banner-card');let sx=0,dx=0,moved=false;card.onpointerdown=e=>{sx=e.clientX;dx=0;moved=false;card.setPointerCapture?.(e.pointerId)};card.onpointermove=e=>{if(!sx)return;dx=e.clientX-sx;if(Math.abs(dx)>5)moved=true;card.style.transform=`translateX(${Math.max(-145,Math.min(145,dx))}px)`;el.classList.toggle('swipe-left',dx<0);el.classList.toggle('swipe-right',dx>0)};card.onpointerup=()=>{card.style.transform='';const action=dx>78?'next':dx<-78?'end':null;el.classList.remove('swipe-left','swipe-right');sx=0;dx=0;if(action==='next')nextNavigationTarget();else if(action==='end')endNavigation()};card.onclick=e=>{if(moved){e.preventDefault();return}showNavigationSteps()}}
...
function endNavigation(){if(overlay.classList.contains('nav-steps-mode'))closeNavigationSteps();stopGpsWatch();navRouteDetail=null;state.nav=false;state.navVisible=false;state.activeMenu=null;$('#navBanner')?.remove();state.view='poi';setSheet('closed');renderDayMap(false);updateTestBadge();syncActiveMenus();toast('Navigáció befejezve')}
...
function showDays(){
 ...
 overlay.classList.add('days-mode');overlay.classList.remove('days-compact');overlay.classList.remove('hidden');document.body.classList.add('overlay-fullscreen');mainNav.classList.remove('joined-panel');hydrateOnlineImages(overlayBody);syncActiveMenus();
 ...
 $$('[data-daychoose]').forEach(b=>b.onclick=()=>{state.day=+b.dataset.daychoose;save();renderDayMap(true);closeOverlay();toast((state.day+1)+'. nap aktív')});
 requestAnimationFrame(()=>{const c=$(`[data-daycard="${state.day}"]`);c?.scrollIntoView({inline:'center',block:'nearest'});updateDots()});
 const dh=$('#daysHandle');if(dh){let sy=0;dh.onpointerdown=e=>{sy=e.clientY;dh.setPointerCapture?.(e.pointerId)};dh.onpointerup=e=>{const dy=e.clientY-sy;sy=0;if(Math.abs(dy)<7){closeOverlay();showDaysCompact()}else if(dy>34)closeOverlay()}}
}
...
function showDaysCompact(){
 ...
 overlay.classList.add('days-compact');overlay.classList.remove('days-mode','standard-full');overlay.classList.remove('hidden');document.body.classList.remove('overlay-fullscreen');mainNav.classList.remove('hide');mainNav.classList.add('joined-panel');logo.classList.remove('hide');hydrateOnlineImages(overlayBody);syncActiveMenus();
 ...
 $$('[data-daycompact]').forEach(b=>{let sx=0,sy=0;b.onpointerdown=e=>{sx=e.clientX;sy=e.clientY};b.onpointerup=e=>{if(Math.abs(e.clientX-sx)>9||Math.abs(e.clientY-sy)>9)return;state.day=+b.dataset.daycompact;save();renderDayMap(true);closeOverlay();toast((state.day+1)+'. nap aktív')};b.onclick=e=>e.preventDefault()});
 requestAnimationFrame(()=>{const c=$(`[data-daycompact="${state.day}"]`);c?.scrollIntoView({inline:'center',block:'nearest'});update()});
 const h=$('#daysCompactHandle');let sy=0;h.onpointerdown=e=>{sy=e.clientY;h.setPointerCapture?.(e.pointerId)};h.onpointerup=e=>{const dy=sy-e.clientY;sy=0;if(dy>18||Math.abs(dy)<7){closeOverlay();showDays()}else if(dy<-18){closeOverlay()}}
}
function showDayQuickMenu(){showDays()}

function hardCloseSheet(){state.sheet='closed';document.body.classList.remove('sheet-fullscreen');document.documentElement.style.setProperty('--sheet-h','0px');sheet.className='sheet state-closed view-'+state.view;sheetBody.innerHTML='';expandedNav.classList.add('hidden');mainNav.classList.remove('joined-panel');$('#mapControls').classList.remove('hidden-by-sheet')}
function hardCloseOverlay(){overlay.classList.add('hidden');overlay.classList.remove('days-mode','days-compact','standard-full','nav-steps-mode');overlayBody.innerHTML='';$('#overlayNav')?.classList.add('hidden');document.body.classList.remove('overlay-fullscreen');state.overlayReturn=null}
function clearTransientPanels(){clearHoldOutside?.();const wasAdding=state.adding;state.adding=false;if(wasAdding){state.sonar=false;state.sonarPins=[];pauseSonarExpiry()}hardCloseSheet();hardCloseOverlay();const hm=$('#holdMenu');hm.classList.add('hidden');hm.classList.remove('wide','quick');mainNav.classList.remove('joined-panel');$('#notifDrawer').classList.remove('open');mainNav.classList.remove('hide');logo.classList.remove('hide');if(wasAdding&&map)renderDayMap(false)}
function returnHome(){clearTransientPanels();state.activeMenu=state.nav?'navigation':null;if(state.nav&&state.selected){state.navVisible=true;renderNavBanner(state.selected);drawNavigationRoute(state.selected)}syncActiveMenus()}
function openFunction(name){
 ...
 if(current===name){returnHome();return}
 ...
 else if(name==='favorites')showFavorites('favorites');
...
function settingsHTML(){return `<div class="settings-section"><h2>🇹🇷 Antalya · Utazás beállításai</h2><div class="setting-card"><b>Tempó</b><input id="paceSlider" class="pace" type="range" min="1" max="5" value="${state.pace}"><div class="setting-row"><small>Nagyon laza</small><b id="paceLabel">${paceName(state.pace)}</b><small>Veretős</small></div></div><div class="setting-card"><b>Közlekedés</b><div class="choice-row" style="margin-top:10px">${['Gyalog','Tömegközlekedés','Autó','Vegyes'].map(x=>`<button class="choice ${state.transport===x?'active':''}" data-transport="${x}">${x}</button>`).join('')}</div></div><div class="setting-card"><div class="setting-row"><div><b>Megérkezés érzékelése</b><small>GPS alapján jelez, de nem jelöli automatikusan késznek.</small></div><button class="toggle ${state.arrivalDetect?'on':''}" data-arrival-toggle aria-label="Megérkezés érzékelése"></button></div></div><div class="setting-card test-environment"><div><b>Tesztkörnyezet</b><small style="display:block;color:var(--muted);margin-top:4px">Antalya-szimuláció · Build ${APP_VERSION}. A térképen nincs külön béta jelvény.</small></div></div><div class="setting-card"><div><b>Offline Antalya</b><small style="display:block;color:var(--muted);margin-top:4px">POI, Guide, útiterv és helyi fotó-fallback a csomagban. Az első online indítás után a térképmotor és a megnyitott csempék is gyorsítótárazódnak.</small></div><div class="setting-row"><span>Offline tartalom előkészítve</span><b style="color:#63cf8b">✓</b></div></div></div><div class="settings-section"><h2>Companion · Általános</h2><div class="setting-card"><b>Téma</b><div class="choice-row" style="margin-top:10px">${['dark','light','auto'].map(x=>`<button class="choice ${state.theme===x?'active':''}" data-theme="${x}">${x==='dark'?'Sötét':x==='light'?'Világos':'Automatikus'}</button>`).join('')}</div></div><div class="setting-card"><div class="setting-row"><div><b>Utazás előtt / Közelgő</b><small>Indulás előtti és következő napi ellenőrzések.</small></div><button class="choice" data-upcoming>Megnyitás</button></div></div></div>`}
function paceName(x){return ['','Nagyon laza','Laza','Kiegyensúlyozott','Aktív','Veretős'][x]}
function bindSettings(){$('#paceSlider').oninput=e=>{state.pace=+e.target.value;$('#paceLabel').textContent=paceName(state.pace);localStorage.setItem('tc_pace',state.pace)};$$('[data-transport]').forEach(b=>b.onclick=()=>{state.transport=b.dataset.transport;localStorage.setItem('tc_transport',state.transport);showSettings()});$$('[data-theme]').forEach(b=>b.onclick=()=>{state.theme=b.dataset.theme;localStorage.setItem('tc_theme',state.theme);applyTheme();showSettings()});const at=$('[data-arrival-toggle]');if(at)at.onclick=()=>{state.arrivalDetect=!state.arrivalDetect;localStorage.setItem('tc_arrival_detect',state.arrivalDetect?'1':'0');at.classList.toggle('on',state.arrivalDetect)};$('[data-upcoming]').onclick=showUpcoming}
...
function showFavorites(source='favorites'){const ids=[...state.favorites];state.overlayReturn=['route','sonar'].includes(source)?{menu:source,view:source==='route'?'route':'sonar',sheet:'two'}:null;state.activeMenu=source==='favorites'?'favorites':source;openOverlay('Mentett helyek','Kedvencek · útitervhez vagy Sonarhoz',`<div class="favorites favorites-stable">${ids.length?ids.map(id=>{const p=POIS[id];return `<button class="fav-card" data-fav="${id}">${imgTag(p)}<span><b>${esc(p.name)}</b><small>${esc(p.sub)}</small></span><i>›</i></button>`}).join(''):'<div class="setting-card">Még nincs mentett hely. Egy POI adatlapján a ♡ gombbal tudsz menteni.</div>'}</div>`);$$('[data-fav]').forEach(x=>x.onclick=()=>{const id=x.dataset.fav;const origin=source;state.overlayReturn=null;hardCloseOverlay();state.selected=id;state.activeMenu=origin==='favorites'?'favorites':origin;state.view='poi';setSheet('mini');renderDayMap(false)})}
...
function openOverlay(title,sub,html){suspendNavBanner();hardCloseSheet();$('#overlayClose').onclick=closeOverlay;$('#overlayBack').onclick=closeOverlay;$('#overlayTitle').textContent=title;$('#overlaySubtitle').textContent=sub||'';overlayBody.innerHTML=html;overlay.classList.remove('hidden');overlay.classList.add('standard-full');document.body.classList.add('overlay-fullscreen');mainNav.classList.remove('hide','joined-panel');logo.classList.remove('hide');$('#overlayNav')?.classList.add('hidden');hydrateOnlineImages(overlayBody);syncActiveMenus()}
function closeOverlay(){const ret=state.overlayReturn;state.overlayReturn=null;hardCloseOverlay();mainNav.classList.remove('hide','joined-panel');logo.classList.remove('hide');if(ret&&['route','sonar'].includes(ret.menu)){state.activeMenu=ret.menu;state.view=ret.view;setSheet(ret.sheet,ret.view)}else{state.activeMenu=state.nav?'navigation':null;$('#mapControls').classList.remove('hidden-by-sheet');if(state.nav&&state.selected){state.navVisible=true;renderNavBanner(state.selected);drawNavigationRoute(state.selected)}}syncActiveMenus()}
...
function showNavigationMenu(){clearTransientPanels();state.activeMenu='navigation';syncActiveMenus();const next=currentDay().route.find(id=>!state.done.has(id)&&!state.skipped.has(id))||currentDay().route[0];showHold([
 {label:state.nav?'Companion · Lépések':'Companion · térkép',icon:'➜',act:()=>state.nav?showNavigationSteps():navigationMain()},{label:'Apple · Tömegköz.',icon:'',act:()=>next&&openExternalTransit(next,'apple')},{label:'Google · Tömegköz.',icon:'G',act:()=>next&&openExternalTransit(next,'google')},
 {label:'Szállás',icon:'⌂',act:()=>showNavigationChoice('hotel')},{label:'Saját pozíció',icon:'◎',act:locate},{label:state.nav?'Navigáció vége':'Mai útvonal',icon:state.nav?'×':'⌁',act:()=>state.nav?endNavigation():fitRoute()}
],'wide')}
let holdOutsideHandler=null;function clearHoldOutside(){if(holdOutsideHandler){document.removeEventListener('pointerdown',holdOutsideHandler,true);holdOutsideHandler=null}}
function showHold(items,mode='wide'){const el=$('#holdMenu');clearHoldOutside();hideHold();mainNav.classList.add('joined-panel');el.classList.add(mode);el.innerHTML=items.map((x,i)=>`<button data-hold="${i}">${x.icon?`<span>${x.icon}</span>`:''}<b>${esc(x.label)}</b></button>`).join('');el.classList.remove('hidden');$$('[data-hold]').forEach(b=>b.onclick=e=>{e.stopPropagation();const act=items[+b.dataset.hold].act;clearHoldOutside();hideHold();act()});holdOutsideHandler=e=>{if(!el.contains(e.target)&&!e.target.closest('.main-tab')){clearHoldOutside();hideHold()}};setTimeout(()=>{if(holdOutsideHandler)document.addEventListener('pointerdown',holdOutsideHandler,true)},30)}
...
function applyMainIcons(){const mapI={mainItinerary:'route',mainDays:'days',mainFavorites:'heart',mainSettings:'settings',mainNotifications:'bell',mainNavigation:'nav'};Object.entries(mapI).forEach(([id,n])=>{const el=$('#'+id+' .ico');if(el)el.innerHTML=uiIcon(n)});logo.querySelector('.logo-dual-hint')?.remove()}
function bindUI(){
 applyMainIcons();
 ...
 longPress($('#mainItinerary'),()=>openFunction('route'),()=>{clearTransientPanels();closeNotifications(true);suspendNavBanner();state.activeMenu='route';state.view='route';setSheet('full','route');syncActiveMenus()});
 longPress($('#mainDays'),()=>openFunction('days'),()=>{clearTransientPanels();showDays()});
 $('#mainFavorites').onclick=()=>openFunction('favorites');
 ...
 longPress(logo,()=>{const keep=(state.sheet!=='closed'&&state.view==='sonar')?'sonar':state.activeMenu;runSonar();if(!(state.sheet!=='closed'&&state.view==='sonar')){state.activeMenu=keep;syncActiveMenus()}},()=>{clearTransientPanels();closeNotifications(true);suspendNavBanner();state.activeMenu='sonar';openSonarMode(false)});
 let sy=0,sh=0,dragging=false,moved=false;
 const sheetDrag=$('#sheetDrag');
 sheetDrag.addEventListener('pointerdown',e=>{if(state.sheet==='closed')return;dragging=true;moved=false;sy=e.clientY;sh=heightFor(state.sheet);sheet.classList.add('dragging');sheetDrag.setPointerCapture?.(e.pointerId)});
 window.addEventListener('pointermove',e=>{if(!dragging)return;const dy=sy-e.clientY;if(Math.abs(dy)>4)moved=true;const h=Math.max(90,Math.min(innerHeight,sh+dy));document.documentElement.style.setProperty('--sheet-h',h+'px')});
 window.addEventListener('pointerup',()=>{if(!dragging)return;dragging=false;sheet.classList.remove('dragging');
   if(!moved){
     if(state.view==='guide'){state.view='poi';setSheet('mini');return}
     if(state.view==='route'){setSheet(state.sheet==='full'?'two':'full','route');return}
     if(['sonar','add','savedadd'].includes(state.view)){setSheet('closed');return}
     if(state.view==='poi'){setSheet(state.sheet==='mini'?'two':'mini','poi');return}
   }
   const h=parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--sheet-h'))||heightFor(state.sheet);
   let snaps=state.view==='route'?[['closed',0],['two',heightFor('two')],['full',heightFor('full')]]:['sonar','add','savedadd'].includes(state.view)?[['closed',0],['two',heightFor('two')]]:[['closed',0],['mini',heightFor('mini')],['two',heightFor('two')],['full',heightFor('full')]];
   const best=snaps.reduce((a,b)=>Math.abs(b[1]-h)<Math.abs(a[1]-h)?b:a);
   if(best[0]==='full'&&state.view==='poi')state.view='guide';setSheet(best[0]);
 });
 window.addEventListener('online',()=>{setOfflineState();toast('Online kapcsolat helyreállt')});window.addEventListener('offline',()=>{setOfflineState();toast('Offline mód · GPS + letöltött tartalom')});
}
function setOfflineState(){document.body.classList.toggle('is-offline',!navigator.onLine)}
function init(){applyTheme();bindUI();renderNotifications();updateTestBadge();setSheet('closed');syncActiveMenus();setOfflineState();if(typeof L==='undefined'){toast('Offline térképmotor nem érhető el · csatlakozz egyszer internetre');return}initMap();window.addEventListener('pageshow',()=>{const id=sessionStorage.getItem('tc_resume_poi');sessionStorage.removeItem('tc_resume_poi');if(id&&POIS[id]){state.selected=id;state.view='poi';setSheet('mini','poi');renderDayMap(false)}});if('serviceWorker'in navigator)navigator.serviceWorker.register('./service-worker.js').catch(()=>{});setTimeout(()=>prewarmPoiPhotos(),1400);}
init();

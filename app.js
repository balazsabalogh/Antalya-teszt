const APP_VERSION='0.7.6-polish-content';
...
async function hydrateOnlineImages(root=document){
 ...
 await Promise.all(imgs.map(async img=>{const id=img.dataset.poiImg;if(!id||(!WIKI_PHOTO[id]&&!PHOTO_QUERY[id]))return;const url=await fetchWikiPhoto(id);if(!url)return;img.src=url;img.dataset.online='1';const badge=img.closest('.photo-wrap')?.querySelector('.photo-badge');if(badge){badge.hidden=false;badge.textContent='Wikimedia'}}));
}
...
],nav:false,navVisible:false,userPos:null,adding:false,notifStripDismissed:false,filters:new Set(),activeMenu:null,planSnapshots:JSON.parse(localStorage.getItem('tc_plan_snapshots')||'[]')};
...
function syncActiveMenus(){
 ...
 const mainMap={itinerary:'route',days:'days',notifications:'notifications',navigation:'navigation'};
...
function updateTestBadge(){const b=$('#tripTestBadge');if(b)b.classList.toggle('show',!!state.userPos&&!gpsInTrip()&&!state.nav)}
...
const sheetHeights={closed:0,mini:158,one:()=>Math.round(innerHeight*.43),two:()=>Math.round(innerHeight*.62),full:()=>innerHeight};
...
function setSheet(s,view){
 ...
 if(view==='route'&&s==='one')s='two';
 state.sheet=s;if(view)state.view=view;
 if(s!=='closed'&&state.view==='route')state.activeMenu='route';
 else if(s!=='closed'&&state.view==='sonar')state.activeMenu='sonar';
 else if(s==='closed'&&!state.nav&&overlay.classList.contains('hidden'))state.activeMenu=null;
 const h=heightFor(s);document.documentElement.style.setProperty('--sheet-h',h+'px');
 sheet.className='sheet state-'+s+(s==='closed'?'':' open')+(s==='full'?' fullscreen':'');
 const expanded=s!=='closed';expandedNav.classList.toggle('hidden',!expanded);mainNav.classList.toggle('hide',expanded);logo.classList.toggle('hide',expanded);
 const ctr=$('#mapControls');const large=['two','full'].includes(s);ctr.classList.toggle('hidden-by-sheet',large);ctr.style.bottom=`calc(${s==='closed'?62:h+2}px + var(--safe-bottom) + 8px)`;
 renderExpandedNav();renderSheet();
 sheet.querySelectorAll('.sheet-close-all').forEach(x=>x.remove());if(s==='full'&&!sheet.querySelector('.guide-close')){const x=document.createElement('button');x.className='sheet-close-all';x.textContent='×';x.setAttribute('aria-label','Panel bezárása');x.onclick=()=>setSheet('closed');sheet.appendChild(x)}
...
const PANEL_NAV_ITEMS=[['route','Útiterv','route'],['days','Napok','days'],['sonar','Sonar','sonar'],['settings','Beáll.','settings'],['bell','Értes.','notifications'],['heart','Kedvenc','favorites']];
...
function bindPanelNav(root=document){root.querySelectorAll('[data-exp]').forEach(b=>b.onclick=()=>{if(b.dataset.exp==='notifications'&&$('#notifDrawer').classList.contains('open')){closeNotifications();return}openFunction(b.dataset.exp)})}
function renderExpandedNav(){if(!expandedNav.classList.contains('hidden')){expandedNav.innerHTML=navMarkup();bindPanelNav(expandedNav)}const on=$('#overlayNav');if(on&&!on.classList.contains('hidden')){on.innerHTML=navMarkup();bindPanelNav(on)}syncActiveMenus()}
function renderSheet(){if(state.sheet==='closed'){sheetBody.innerHTML='';return}if(state.view==='route'){renderRouteSheet();return}if(state.view==='sonar'){renderSonarSheet();return}if(state.view==='navigation'){renderNavSheet();return}if(state.view==='add'){renderAddSheet();return}if(state.view==='guide'){renderGuideSheet();return}renderPoiSheet()}
...
function renderPoiSheet(){const id=state.selected||currentDay().route[0],p=POIS[id];if(!p)return;
 if(state.sheet==='mini'){sheetBody.innerHTML=miniPoi(p);$('[data-more]').onclick=()=>setSheet('one','poi');hydrateOnlineImages(sheetBody);return}
 if(state.sheet==='one'){
   sheetBody.innerHTML=`${miniPoi(p)}${poiActions(id)}<div class="section-title">Környék szűrők</div><div class="filters">${filterHTML()}</div>`;
   $('[data-more]').onclick=()=>setSheet('two','poi');bindPoiActions(id);bindFilters();hydrateOnlineImages(sheetBody);return
 }
 if(state.sheet==='two'){
   sheetBody.innerHTML=`${imgTag(p,'poi-detail-hero')}<div class="poi-detail-head"><div><h2>${esc(p.name)}</h2><p>${esc(p.sub)}</p><span class="tag good">${p.hours}</span> <span class="tag">${p.duration}</span></div><button class="expand detail-expand" data-full-guide aria-label="Guide megnyitása">⌃</button></div>${poiActions(id)}<div class="guide-block"><h3>Miért érdemes?</h3><p>${esc(p.why)}</p></div><div class="section-title">Az útvonalban</div><div class="route-context">${routeRows(id)}</div>`;
   bindPoiActions(id);$('[data-full-guide]').onclick=()=>{state.view='guide';setSheet('full')};bindRouteRows();hydrateOnlineImages(sheetBody);return
 }
...
function bindFilters(){$$('[data-filter]').forEach(b=>b.onclick=()=>{const t=b.dataset.filter;if(state.filters.has(t))state.filters.delete(t);else state.filters.add(t);renderFilterPins();$$(`[data-filter="${t}"]`).forEach(x=>x.classList.toggle('active',state.filters.has(t)))})}
...
function renderRouteSheet(){const d=currentDay();
 if(state.sheet!=='full'){
  const next=d.route.find(id=>!state.done.has(id)&&!state.skipped.has(id))||d.route[0];sheetBody.innerHTML=`<div class="planner-head"><div><h2>${d.id}. nap · ${esc(d.title)}</h2><small>${[...state.done].filter(x=>d.route.includes(x)).length}/${d.route.length} kész</small></div><span class="tag">${esc(d.transport)}</span></div><div class="route-context route-large">${routeRows(next)}</div>`;bindRouteRows();hydrateOnlineImages(sheetBody);return
 }
 sheetBody.innerHTML=`<div class="planner-head fullscreen-head"><div><h2>${d.id}. nap · ${esc(d.title)}</h2><small>${d.date} · teljes útvonaltervező</small></div><div class="head-actions"><button class="choice" data-save-plan>Mentés</button><button class="choice" data-restore-plan>↶</button></div></div><div class="full-planner">${plannerCards()}</div><button class="add-stop" id="addStop">＋ Állomás hozzáadása</button>`;
 $('[data-save-plan]').onclick=()=>{snapshotPlan('Kézi mentés');toast('✓ Tervállapot elmentve')};$('[data-restore-plan]').onclick=restoreLatestSnapshot;$('#addStop').onclick=()=>enterAddMode();bindPlannerGestures();hydrateOnlineImages(sheetBody);
}
...
function bindPlannerGestures(){$$('.planner-card').forEach(card=>{let sx=0,dx=0,drag=false;card.addEventListener('pointerdown',e=>{sx=e.clientX;dx=0;drag=true;card.setPointerCapture?.(e.pointerId)});card.addEventListener('pointermove',e=>{if(!drag)return;dx=e.clientX-sx;if(Math.abs(dx)>8)card.style.transform=`translateX(${Math.max(-105,Math.min(105,dx))}px)`});card.addEventListener('pointerup',()=>{if(!drag)return;drag=false;const id=card.dataset.plan;if(dx>72){snapshotPlan('Megnéztem előtt');state.done.add(id);state.skipped.delete(id);toast('✓ Megnézve')}else if(dx<-72){snapshotPlan('Kihagyás előtt');state.skipped.add(id);toast('Kihagyva')}card.style.transform='';save();renderSheet();renderDayMap()});card.onclick=()=>{state.selected=card.dataset.plan;setSheet('two','poi')}})}
...
function runSonar(){
  ...
  state.notifStripDismissed=false;state.notifications.unshift({id:'sonar-latest',priority:'useful',title:`${candidates.length} hely van a közelben`,text:names,action:'sonarHistory',pins:[...candidates]});
...
function openSonarMode(){
  state.activeMenu='sonar';state.sonar=true;pauseSonarExpiry();
  const origin=effectiveOrigin();
...
function bindSonarSwipes(){
  $$('.sonar-result').forEach(w=>{const card=w.querySelector('.sonar-result-card');let sx=0,dx=0,moved=false;
    ...
    card.onpointerup=()=>{const id=w.dataset.sonarResult;card.style.transform='';w.classList.remove('to-left','to-right');if(dx>70){addAsNext(id);toast('Következő állomásként hozzáadva')}else if(dx<-70){state.sonarPins=state.sonarPins.filter(x=>x!==id);toast('Elvetve');renderSonarSheet();renderDayMap(false)}sx=0;dx=0};
...
function showDays(){
 suspendNavBanner();setSheet('closed');state.activeMenu='days';logo.classList.add('hide');mainNav.classList.add('hide');$('#mapControls').classList.add('hidden-by-sheet');
 const on=$('#overlayNav');on.classList.remove('hidden');renderExpandedNav();$('#overlayTitle').textContent='';$('#overlaySubtitle').textContent='';
 ...
 overlay.classList.add('days-mode');overlay.classList.remove('hidden');hydrateOnlineImages(overlayBody);syncActiveMenus();
...
function showDaysCompact(){
 ...
 overlay.classList.add('days-compact');overlay.classList.remove('hidden');mainNav.classList.remove('hide');logo.classList.remove('hide');hydrateOnlineImages(overlayBody);syncActiveMenus();
...
function openFunction(name){state.activeMenu=name;syncActiveMenus();if(name==='notifications'){toggleNotifications();return}closeNotifications(true);if(name==='route'&&state.view==='route'&&state.sheet!=='closed'){setSheet('closed');return}if(!overlay.classList.contains('hidden'))closeOverlay();if(name!=='navigation')suspendNavBanner();if(name==='route'){state.view='route';setSheet('two')}else if(name==='days')showDaysCompact();else if(name==='sonar')openSonarMode();else if(name==='settings')showSettings();else if(name==='favorites')showFavorites()}
function showSettings(){state.activeMenu='settings';openOverlay('Beállítások','Antalya · utazás + Companion',settingsHTML());bindSettings();syncActiveMenus()}
...
function showFavorites(){state.activeMenu='favorites';const ids=[...state.favorites];openOverlay('Mentett helyek','Utazás · Közelben · Összes',`<div class="choice-row" style="margin-bottom:12px"><button class="choice active">Utazás</button><button class="choice">Közelben</button><button class="choice">Összes</button></div><div class="favorites">${ids.length?ids.map(id=>{const p=POIS[id];return `<div class="fav-card" data-fav="${id}">${imgTag(p)}<div><b>${esc(p.name)}</b><small>${esc(p.sub)}</small></div><button class="iconbtn">›</button></div>`}).join(''):'<div class="setting-card">Még nincs mentett hely. A POI-knál a ♡ gombbal ments.</div>'}</div>`);$$('[data-fav]').forEach(x=>x.onclick=()=>{closeOverlay();selectPoi(x.dataset.fav)})}
...
function bindUI(){
 ...
 $('#mainNotifications').onclick=()=>{closeCompactIfOpen();state.activeMenu='notifications';toggleNotifications()};
 longPress($('#mainDays'),showDaysCompact,showDays);
 longPress($('#mainItinerary'),()=>{if(!$('#holdMenu').classList.contains('hidden')&&$('#holdMenu').classList.contains('quick')){hideHold();return}showHold([{label:'Következő',icon:'➜',act:()=>{const n=currentDay().route.find(x=>!state.done.has(x));if(n)selectPoi(n)}},{label:'+ Állomás',icon:'＋',act:enterAddMode},{label:'Teljes terv',icon:'≡',act:()=>{state.view='route';setSheet('full')}},{label:'Mai állapot',icon:'✓',act:()=>toast(`${[...state.done].filter(x=>currentDay().route.includes(x)).length}/${currentDay().route.length} kész`)}],'quick')},()=>{hideHold();state.view='route';setSheet('two')});
...
function init(){applyTheme();initMap();bindUI();renderNotifications();setSheet('closed');syncActiveMenus();if('serviceWorker'in navigator)navigator.serviceWorker.register('./service-worker.js').catch(()=>{});setTimeout(()=>prewarmPoiPhotos(),1400);}
init();
